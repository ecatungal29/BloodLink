...
function getEffectiveStatus(item: InventoryItem): string {
	if (item.last_updated) {
		const hoursSince =
			(Date.now() - new Date(item.last_updated).getTime()) / 36e5;
...
export default function InventoryPage() {
	...
	const fetchInventory = useCallback(async () => {
		...
		try {
			const { data } = await api.get<
				PaginatedResponse<InventoryItem> | InventoryItem[]
	...
	return (
		<div className="space-y-5">
			{/* Filter bar */}
			<div className="bg-white rounded-2xl border border-slate-100 shadow-sm px-5 py-4 flex flex-wrap items-center gap-4 text-slate-700">
				<FilterSelect
					label="Component"
