...
export default function SearchPage() {
	...
	const handleSearch = async () => {
		...
		try {
			const { data } = await api.get<HospitalSearchResult[]>(
				`/api/donations/search/?${params}`,
			);
			setResults(data || []);
		} catch {
			setResults([]);
	...
	const handleSendInquiry = async () => {
		if (!inquiryTarget || !filters.component || !filters.abo || !filters.rh)
			return;
		setSubmitting(true);
		setInquiryError("");
	...
	return (
		<div className="space-y-5">
			{/* Filter panel */}
			<div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 text-slate-600">
				...
				{!canSearch && searched && (
					<p className="text-xs text-amber-600 mb-3">
						Component, ABO group, and Rh factor are required to search.
					</p>
				)}
				<button
			...
			{searched && (
				<div className="space-y-3">
					...
					:	results.map((hospital) => {
							const invStatus =
								hospital.matched_inventory?.availability_status || "none";
							const badgeClass =
								AVAILABILITY_BADGE[invStatus] || "bg-slate-100 text-slate-500";
							const lastUpdated =
								hospital.matched_inventory?.last_updated ?
									new Date(
										hospital.matched_inventory.last_updated,
								...
								<div
									...
									<div className="flex-1 min-w-0">
										<div className="flex items-center gap-3 mb-1">
											...
											<span
												className={`inline-flex px-2 py-0.5 rounded-full text-[11px] font-semibold capitalize ${badgeClass}`}>
										...
										<div className="flex flex-wrap items-center gap-4 text-xs text-slate-500">
											...
											{hospital.matched_inventory && (
												<span className="font-medium text-slate-700">
													{hospital.matched_inventory.units_available} units
													available
												</span>
											)}
											{lastUpdated && (
					...
					<div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
						{inquirySent ?
							<div className="text-center py-4">
								...
								<p className="text-sm text-slate-500 mb-5">
									Your blood request has been logged. Hospitals can now respond
									to it.
								</p>
								<button
						...
						:	<>
								...
								<p className="text-sm text-slate-500 mb-1">
									...
									<span className="font-medium text-slate-700">
										{filters.component} {filters.abo}
										{filters.rh}
									</span>
								</p>
								<p className="text-xs text-slate-400 mb-5">
									This will be logged as an open blood request visible to all
								...
								<div className="space-y-4">
										...
										<div>
											...
											<input
												...
												onChange={(e) =>
													setInquiryForm((f) => ({
														...f,
														units_needed: parseInt(e.target.value) || 1,
													}))
												}
									...
									<div>
										...
										<textarea
											...
											onChange={(e) =>
												setInquiryForm((f) => ({ ...f, notes: e.target.value }))
											}
